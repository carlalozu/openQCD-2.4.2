...
static qflt local_plaq_sum_dble(int iw)
{
   ...
   #pragma omp target update to(tms_arr)
   ...
   //memcpy(udb_test, udb, sizeof(udb_test));
   #pragma omp target update to(plaq_uidx_array)
   #pragma omp target update to(udb_test)
   //#pragma omp target update to(udb[0:mysize])

   #pragma omp target data map(to:tms[0:VOLUME])
   {
      ...
      //#pragma omp parallel for private(k,ix,t,n,pa) reduction(sum_qflt : rqsm)
      #pragma omp target teams distribute parallel for private(k,ix,t,n,pa) reduction(sum_qflt : rqsm) //map(to:udb[0:4*VOLUME+7*(BNDRY/4)], tms[0:VOLUME]) collapse(2) num_teams(256)
      //#pragma omp target parallel for reduction(+:pa)
      {
         for (int ix=0;ix<VOLUME;ix++)
         {
            //t=global_time_array(ix, tms_arr);
            t=global_time(ix);
            pa=0.0;
