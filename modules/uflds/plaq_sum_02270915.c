...
static qflt local_plaq_sum_dble(int iw)
{
   ...
   #pragma omp target data map(to:tms[0:VOLUME], udb[0:4*VOLUME+7*(BNDRY/4)])
   {
      MPI_Barrier(MPI_COMM_WORLD);
      time_start=MPI_Wtime();

      #pragma omp parallel for private(k,ix,t,n,pa) reduction(sum_qflt : rqsm)
      //#pragma omp target teams distribute parallel for private(k,ix,t,n,pa) reduction(sum_qflt : rqsm) thread_limit(8) num_teams(168) //thread_limit(256)
