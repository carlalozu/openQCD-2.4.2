...
//static int plaq_uidx_array = malloc(6*VOLUME*4*sizeof(int));
static qflt rqsm_arr[12];
static qflt rqsm;
...
static double plaq_dble(int n,int ix)
{
   ...
   su3_dble wd1 = su3xsu3_test(udb+ip[0],udb+ip[1]);
   //su3_dble wd2 = su3dagxsu3dag_test(udb+ip[3],udb+ip[2]);
...
static double plaq_dble_initialized(int n,int ix, double sm, su3_dble wd1, su3_vector_dble psi, su3_vector_dble chi)
{
   int *ip = &plaq_uidx_array[n*VOLUME*4 + ix*4];
   //double sm;
   //su3_dble *wd1 = __runtime_alloc(18*sizeof(double));// ALIGNED16;
   ...
   //su3_dble wd2 = su3dagxsu3dag_test(udb+ip[3],udb+ip[2]);
   //wd2 = su3dagxsu3dag_test(udb+ip[3],udb+ip[2]);
   su3xsu3_initialized(udb+ip[0],udb+ip[1],wd1, psi, chi);
   //su3dagxsu3dag(udb+ip[3],udb+ip[2],&wd2);
   //cm3x3_retr(&wd1,&wd2,&sm);
...
static qflt local_plaq_sum_dble(int iw)
{
   ...
   #pragma omp target update to(rqsm_arr)
...
#pragma omp target teams distribute parallel for map(tofrom:pa) private(sm, wd1, psi, chi) //reduction(sum_qflt:rqsm) private(ix,t,n,pa)
   {
      for (ix=0;ix<1;ix++)
      {
         ...
         if ((omp_get_team_num() == 0) && (omp_get_thread_num() == 0)) {
            //pa=plaq_dble(0,2);
            pa=plaq_dble_initialized(0,2, sm, wd1, psi, chi);
            //pa = 15;
         }
         //pa=plaq_dble(0,2);
         //pa = 2.0;
