...
#pragma omp declare target
static int offset(int ix,int mu,int (*iup)[4])
{
   int iy,ib;
...
#pragma omp parallel private(k,l,ib,ib0,ib1,iy,iz,nu)
         {
            ...
            for (ib=ib0;ib<ib1;ib++)
            {
               ...
               {
                  nu=l+(l>=mu);
                  iuk[3*(ib+nu0)+l]=offset(iz,nu,iup);
               }
            }
         }
      }
...
void plaq_uidx(int n,int ix,int *ip)
{
   ...
   ip[2]=offset(ix,nu,iup);
   iy=iup[ix][nu];

   if (iy<VOLUME)
      ip[3]=offset(iy,mu,iup);
   else
   {
      if (iy<(VOLUME+(BNDRY/2)))
         ic=iy-VOLUME-nfc[nu];
