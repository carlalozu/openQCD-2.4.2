...
int main(int argc,char *argv[])
{
   ...
   if (my_rank==0)
   {
      flog=freopen("time1_mpi1_openmp12_16x16x16x16_param1_run11.log","w",stdout);
   ...
   wdt=1e6*wdt/((double)(nt));

   if (my_rank==0)
   {
      printf("Time call to plaq_sum_dble:\n");
      printf("%f micro sec\n\n",wdt);
      printf("Time per plaq:\n"); 
      printf("%f micro sec\n\n",(double)(wdt/VOLUME_TRD));
      printf("Number of iterations:\n"); 
      printf("n_iter = %d\n\n",n_iter);
      printf("Number of flop/s:\n"); 
      printf("%f Mega flop/s\n\n",(double)(468*VOLUME_TRD/wdt));  
      fclose(flog);
   }
