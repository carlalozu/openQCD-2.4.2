...
static inline void doublev_init(doublev *x, size_t volume)
{
    ...
    {
        x->volume = 0;
        fprintf(stderr, "Error allocating doublev\n");
        abort();
    }
}
...
{
    /*
     * Map the nested su3_vec_field structs themselves so that the pointer
     * fixup kernel below can write c1/c2/c3 into device-resident memory.
     * Without mapping the structs, the writes in the target region are lost.
     */
#pragma omp target enter data \
    map(to : m_field[0], \
             m_field->c1.base[0 : 3 * m_field->c1.volume], \
