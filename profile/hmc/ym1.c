...
int main(int argc,char *argv[])
{
   ...
   prof_section total_p = {.name = "total"};
   prof_section init_program_p = {.name = "init_program"};
   prof_section compute_p = {.name = "compute"};
   prof_section extra_functions_p = {.name = "extra_functions"};

   mpi_init(argc,argv);
   MPI_Comm_rank(MPI_COMM_WORLD,&my_rank);

   prof_begin(&total_p);
   prof_begin(&init_program_p);
   ...
   if (my_rank==0)
   {
      prof_report(&init_program_p);
      prof_report(&compute_p);
