...
int main(int argc, char *argv[])
{
   ...
   prof_section s_kernel  = {.name = "plaq_sum_dblev"};
   ...
   if (my_rank == 0)
   {
      ...
      printf("\n");
      printf("Total plaquette action (plaq_sum) of the double-precision gauge field SoA\n");
      printf("-------------------------------------------------------------------\n\n");
   ...
   if (my_rank == 0)
   {
      ...
      printf("Flops: %d\n", flops); 
      printf("Total performance for plaq_sum_dblev (GFlops/s): %f\n", (double)(flops * 1e-9 / avg_time)); 
      printf("Time per lattice point & thread for plaq_sum_dblev (sec): %.9f\n", avg_time/((double)(VOLUME_TRD)));
      printf("Result: %f\n\n", result);
